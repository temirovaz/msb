...
if ($_SERVER['REQUEST_METHOD']=='POST' && isset($_REQUEST['SAVE_API']) && check_bitrix_sessid()) {
    ...
    LocalRedirect($currentUrl . "&success=api");
}

// Обработка добавления товара
if ($_SERVER['REQUEST_METHOD']=='POST' && $_REQUEST['ADD_PRODUCT'] && check_bitrix_sessid()) {
    ...
    LocalRedirect($currentUrl . "&success=add");
}

// Обработка удаления товара
if ($_SERVER['REQUEST_METHOD']=='POST' && $_REQUEST['REMOVE_ID'] && check_bitrix_sessid()) {
    $id = (int)$_REQUEST['REMOVE_ID'];
    $currentIds = array_diff($currentIds, [$id]);
    COption::SetOptionString($module_id, 'PRODUCTS_SHOW', serialize($currentIds));
    LocalRedirect($currentUrl . "&success=del");
}

// Выбрать все товары
if ($_SERVER['REQUEST_METHOD']=='POST' && isset($_REQUEST['ALL_PRODUCTS']) && check_bitrix_sessid()) {
    COption::SetOptionString($module_id, 'PRODUCTS_SHOW', serialize(['ALL']));
    LocalRedirect($currentUrl . "&success=all");
}

// Очистить все товары
if ($_SERVER['REQUEST_METHOD']=='POST' && isset($_REQUEST['CLEAR_ALL']) && check_bitrix_sessid()) {
    COption::SetOptionString($module_id, 'PRODUCTS_SHOW', serialize([]));
    LocalRedirect($currentUrl . "&success=clear");
}
    ...
    if ($allSelected):
        ...
        if (empty($currentIds)):
            ...
            foreach($currentIds as $pid) {
                ...
                echo "<form style='display:inline;' method='post'>".bitrix_sessid_post()."<input type='hidden' name='REMOVE_ID' value='$pid'/><button type='submit' style='padding: 3px 10px; background: #dc3545; color: white; border: none; border-radius: 3px; cursor: pointer;'>Убрать</button></form></li>";
            }
        endif;
    endif;
    ?>
